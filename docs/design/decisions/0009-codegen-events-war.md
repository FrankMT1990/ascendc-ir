...
1. **event 分配**：每逻辑通道一个 `EVENT_ID`——每条前向 sync 一条通道（按 producer/consumer/on 集合去重），每个跨 PIPE buffer 的 WAR 释放一条通道。通道总数超 `event_ids` 则拒绝生成。同通道跨迭代复用同一 event（生产/消费双方各自按程序顺序配对）。
